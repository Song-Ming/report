...
The TRL and PEFT libraries were used in a custom training script for finetuning MedGemma while LLaVA-Rad was finetuned by modifying the provided script from the LLaVA-Rad
repository. For both models, the weights of the linear layers of the model were targeted for finetuning. The Adam-W adaptive optimiser was used instead of SGD as with 
the CNN models to increase the convergence speed. For MedGemma, the hyperparameters used were batch size of 32 divided into 8 accumulation steps, max epochs of 5, learning rate 
